"""Tkinter GUI implementation for Bambu2Prusa converter."""
...
class ZipProcessorGUI:
    ...
    def select_input(self):
        ...
        initial_dir = first_existing_dir(self.settings.last_input_dir)
        if initial_dir:
    ...
    def choose_input_dir(self):
        initial_dir = first_existing_dir(self.input_dir_var.get())
        directory = filedialog.askdirectory(initialdir=initial_dir or None)
        if directory:
            self.input_dir_var.set(directory)

    def choose_output_dir(self):
        initial_dir = first_existing_dir(self.output_dir_var.get())
        directory = filedialog.askdirectory(initialdir=initial_dir or None)
        if directory:
...
def main():
    """Main entrypoint for Tkinter GUI."""
    root = Tk()
